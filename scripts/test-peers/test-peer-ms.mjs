 ...
 *
 * Env overrides:
 *   TEST_PEER_NSEC_HEX  — 64-char hex private key. If set, this identity
 *                         is used (and NOT persisted to disk). Useful when
 *                         the SFU's admin UI spawns the peer and wants a
 *                         unique key per spawn. If unset, falls back to
...
//   1. TEST_PEER_NSEC_HEX env — when the admin UI spawns us, it generates
//      a fresh ephemeral key per spawn so multiple test peers don't collide.
//   2. On-disk identity at scripts/.test-peer-ms/identity.json — manual
//      runs share a stable identity so the dex remembers us across restarts.
//   3. New random key, persisted to disk.
let secretKey;
let pubkey;
...
if (envNsec && /^[0-9a-f]{64}$/i.test(envNsec)) {
  ...
  console.log('[test-ms] using TEST_PEER_NSEC_HEX (ephemeral, not persisted)');
} else {
